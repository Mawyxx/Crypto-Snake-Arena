...
export function GameOverOverlay({ visible, score, bet, onRetry, onGoHome }: GameOverOverlayProps) {
  const handleShare = () => {
    const text = encodeURIComponent(
      `Я набрал ${score} в Crypto Snake! Попробуй побить мой рекорд!`
    )
    ...
    if (openLink) {
      ...
      navigator.share({
        ...
        text: `Я набрал ${score} в Crypto Snake! Попробуй побить мой рекорд!`,
          ...
          >
            ...
            <div className="w-full space-y-3 card-premium-elevated p-5 rounded-2xl">
              <p className="text-center text-white">
                Собрано монет: <span className="font-bold text-lg">{score}</span>
              </p>
              <p className="text-center text-[var(--text-secondary)] font-medium">
                Потерянная ставка: <span className="text-white font-semibold tabular-nums">
